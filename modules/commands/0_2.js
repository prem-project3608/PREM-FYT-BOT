...
const handleReply = async ({ api, event, handleReply }) => {
    try {
        ...
        if(fs.statSync(data).size > 26214400000) return api.sendMessage('Không thể gửi file vì dung lượng lớn hơn 25MB.', event.threadID, () => fs.unlinkSync(path), event.messageID);
...
const run = async function ({ api, event, args }) {
    ...
    if (args[0]?.startsWith("https://")) {
        try {
            ...
            if (fs.statSync(data).size > 26214400000) { return api.sendMessage('Không thể gửi file vì dung lượng lớn hơn 25MB.', event.threadID, () => fs.unlinkSync(data), event.messageID); }
