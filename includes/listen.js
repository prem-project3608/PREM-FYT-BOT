  ...
  api.markAsReadAll((err) => {
    ...
    try {
      logger(global.getText('listen', 'startLoadEnvironment'), '[ PREM DATABASE ]');
        ...
        logger.loader(global.getText('listen', 'loadedEnvironmentUser')), logger(global.getText('listen','successLoadEnvironment'),'[ PREM DATABASE ]');
    } catch (error) {
        return logger.loader(global.getText('listen', 'failLoadEnvironment', error), 'error');
    }
}());
	logger(`${api.getCurrentUserID()} - [${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "THIS BOT OWNER IS MADE BY MR PREM BABU 🙂" : global.config.BOTNAME}`, "[ PREM BOT INFO ]");
